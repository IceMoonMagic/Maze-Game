...
func adjust_points(start: Vector2i, end: Vector2i) -> Array[Vector2]:
	...
	if Vector2(start).angle_to_point(end) == 0:  # If horizontal line
		return [
			...
			Vector2(end) * MazeOptions.TILE_SIZE + Vector2.RIGHT * offset
		]
	else:  # If vertical line
		return [
			Vector2(start) * MazeOptions.TILE_SIZE + Vector2.UP * offset,
			Vector2(end) * MazeOptions.TILE_SIZE + Vector2.DOWN * offset
		]
